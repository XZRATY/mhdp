...
def load_provided_proxies(proxies_file):
    ...
    if not proxies:
        logger.warning(f'{cl.RED}У {proxies_file} не найдено прокси - проверьте формат{cl.RESET}')
    else:
        logger.info(f'{cl.YELLOW}Считано {cl.BLUE}{len(proxies)}{cl.YELLOW} прокси{cl.RESET}')
    return proxies
